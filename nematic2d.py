...
Nx, Ny  = 32, 32                               # grid points in x and y
...
for t in np.arange(0, tsteps, dt):
    ...
    if t % (tsteps/100) == 0:
        b = np.square(a).sum(axis=1)  # S^2 = sum(ai^2)
        b = np.sqrt(b); b = np.reshape(b, (Nx, Ny)) 
        
        q0 = -sqrt(1.0/6.0)*a[:,0] + sqrt(1.0/2.0)*a[:,1];
        q1 = sqrt(1.0/2.0)*a[:,2];
        q2 = sqrt(1.0/2.0)*a[:,4];
        q3 = -sqrt(1.0/6.0)*a[:,0] - sqrt(1.0/2.0)*a[:,1];
        q4 = sqrt(1.0/2.0)*a[:,4];
        q5 = sqrt(4.0/6.0)*a[:,0];
        
        Q = np.array([[q0,q1,q2],[q1,q3,q4],[q2,q4,q5]]);
        ...
        print j, len(S)
        S = np.reshape(S, (Nx, Ny))
        T = np.reshape(T, (Nx, Ny))
        clf(); pcolor(xx,yy,b), axis('equal'), axis('tight'), colorbar()
        pause(0.1)
